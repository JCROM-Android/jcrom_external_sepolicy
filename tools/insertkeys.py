    ...
    def __init__(self, path):
        ...
        for line in pkFile:
            ...
            if line.startswith("-----BEGIN CERTIFICATE-----"):
                if inCert:
                    sys.exit("Encountered another BEGIN CERTIFICATE without END CERTIFICATE on line: "
                            + str(lineNo))
            ...
            # else we have started the certicate and need to append the data
                ...
                # We should never hit this assert, if we do an unacounted for state
                # was entered not addressed by the statements above
                assert(False == True)
